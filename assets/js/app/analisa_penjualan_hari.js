...
$(document).ready(function() {
        ...
        table = $('#example').DataTable( {
             "ajax": {
                "url": site_url+"analisa_penjualan/view_hari",
                "dataSrc": ""
              },
            "autoWidth": false,
            ...
            buttons: [
                {
                    extend: 'print',
                    message: 'Laporan Penjualan Per Hari',
            ...
            "footerCallback": function ( row, data, start, end, display ) {
                    ...
                    totalr = api.column( 3 ).data().reduce( function (a, b) {return intVal(a) + intVal(b);}, 0 );
                    total = api.column( 2 ).data().reduce( function (a, b) {return intVal(a) + intVal(b);}, 0 );

                    // Total over this page
                    pageTotalr = api.column( 3, { page: 'current'} ).data().reduce( function (a, b) {return intVal(a) + intVal(b);}, 0 );
                    pageTotal = api.column( 2, { page: 'current'} ).data().reduce( function (a, b) {return intVal(a) + intVal(b);}, 0 );

                    // Update footer
                    $( api.column( 3 ).footer() ).html($.number(pageTotalr,0,',','.') +' ('+ $.number(totalr,0,',','.') +' total)');
                    $( api.column( 2 ).footer() ).html($.number(pageTotal,0,',','.') +' ('+ $.number(total,0,',','.') +' total)');
                    $( api.column( 1 ).footer() ).html($.number(pageTotal/pageTotalr,0,',','.') +' ('+ $.number(total/totalr,0,',','.') +' total)');
                }
        } );
        ...
        $('#search').click(function (){
            ...
            table.ajax.url( site_url+"analisa_penjualan/view_hari/"+start+"/"+end+'/'+outlet ).load();
        });
